...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        if [ "${token_type}" = bpe ]; then
            mkdir /GPFS/data/heyangliu/BrainGPT_data/BPE/bpe_100
            spm_train \
                --input=/GPFS/data/heyangliu/BrainGPT_data/derived/transcriptions/train.txt \
                --vocab_size=100 \
                --model_type=bpe \
                --model_prefix=/GPFS/data/heyangliu/BrainGPT_data/BPE/bpe_100/brain \

            cat /GPFS/data/heyangliu/BrainGPT_data/derived/transcriptions/trans.txt | while read line
            do
                echo " ${line}" | spm_encode --model=/GPFS/data/heyangliu/BrainGPT_data/BPE/bpe_100/brain.model --output_format=id >> /GPFS/data/heyangliu/BrainGPT_data/derived/transcriptions/trans_100.txt
            done
        fi
    fi
else
    ...
    if [ ${stage} -le 10 ] && [ ${stop_stage} -ge 10 ]; then
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.asr_train \
                ...
                --output_dir "${_logdir}/stats.JOB" \
                ${_opts} ${asr_args} || { cat "${_logdir}"/stats.1.log; }
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ]; then
        ...
        for dset in "${valid_set}" ${test_sets}; do
            ...
            for f in text; do
                for i in $(seq "${_nj}"); do
                    sed -i "s/ <hesitation>//g" ${_logdir}/output.${i}/1best_recog/text
                    sed -i "s/<hesitation> //g" ${_logdir}/output.${i}/1best_recog/text
                done
            done
        done
    ...
    if [ ${stage} -le 13 ] && [ ${stop_stage} -ge 13 ]; then
        ...
        for dset in "${valid_set}" ${test_sets}; do
            _data="${data_feats}/${dset}"
            _dir="${asr_exp}/${inference_tag}/${dset}"
            _werdir="${_dir}/score_wer"
            _biasing_dir="data/biasing"
            for _type in wer cer ter cwer oov ; do
                ...
                if [ "${_type}" = wer ]; then
                    ...
                    paste \
                        ...
                        <(<"${_data}/utt2spk" awk '{ print "(" $2 "-" $1 ")" }') \
                            >"${_scoredir}/hyp.trn"
                    #1line
                    sed -i "s/ <hesitation>//g" ${_scoredir}/hyp.trn
                    sed -i "s/<hesitation> //g" ${_scoredir}/hyp.trn
                ...
                elif [ "${_type}" = cwer ]; then
                    for num in 1 5 10 20 ; do
                      _scoredir="${_dir}/score_cwer_${num}"
                      mkdir -p "${_scoredir}"
                      python3 local/cwer.py --biasing_list ${_biasing_dir}/biasing_${num}.txt --in_path ${_werdir} --out_path ${_scoredir}
                    ...
                    _scoredir="${_dir}/score_oov"
                    mkdir -p "${_scoredir}"
                    python3 local/cwer.py --biasing_list ${_biasing_dir}/oov.txt --in_path ${_werdir} --out_path ${_scoredir}
